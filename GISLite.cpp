    ...
    : QMainWindow(parent),ui(new Ui::GISLiteClass)
{
    ...
    CommandPolygon* CAPG = new CommandPolygon;
    CommandCircle* CAC = new CommandCircle;
    ...
    QAction* action2g = addbar->addAction(QIcon(":/GISLite/icons/t-32.png"), "Character");
    ...
    addToolBar(editbar);editbar->setVisible(false);
    addToolBar(stylebar); stylebar->setVisible(false);
    ...
    if (selectedItem)
    {
        if (selectedItem->text().left(10) == "Merge with")
        {
            for (int i = 0; i < LayerList.count(); i++)
            {
				...
				{
					LayerList[i]->getVector().append(LayerList[selectSetIndex]->getVector());
					LayerList.erase(LayerList.begin() + selectSetIndex-1);	
					break;
				}
            }            
        }
        else if (selectedItem->text() == "Set name")
        {
		    bool ok;
			QString text = QInputDialog::getText(this, tr("QInputDialog::getText()"),tr("Layer Name:"), QLineEdit::Normal,"", &ok);
			if (ok && !text.isEmpty())
			{
				LayerList[selectSetIndex]->setName(text);
				listWidget->item(selectSetIndex)->setText(text);
			}
		}
		else if (selectedItem->text() == "Delete")
		{
			LayerList.erase(LayerList.begin() + selectSetIndex);
			listWidget->takeItem(selectSetIndex);
        }
    }
}